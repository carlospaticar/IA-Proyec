...
def chat(model,vectorizer,unique_answers):
    ...
    print("\n 💬 chatbot iniciado. Escriba salir para terminar \n")
    while True:
            user = input("Tu: ").strip()
...
def main():
    ...
    while True:
        ...
        if opcion == "1":
            if model is None:
                print("\n ⚠️ No hay modelo entrenaado. Entrenalo primero. ")
            else:
                chat(model,vectorizer,unique_answers)

        elif opcion == "2":
            print("\n 🔁 Reentrenando el modelo el modelo con los datos... ")
